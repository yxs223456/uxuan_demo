...
class Pintuans extends Base{
	...
	public function add(){
		...
		if((int)$data['tuanNum']<=0)return WSTReturn('拼团人数必须大于0');
        if((int)$data['tuanSum']<=0)return WSTReturn('开团数量必须大于0');
	...
	public function unSaleRefund($tuanId){
		/*$pusers = Db::name('pintuan_users pu')->join("__USERS__ u","pu.userId=u.userId")
				...
				->where(["p.tuanStatus"=>1,"p.tuanId"=>$tuanId])
	...
	*/
	public function allow(){	
		...
		$rs = $this->alias('gu')
		           ...
		           ->join('__SHOPS__ s','g.shopId=s.shopId','left')
		           ->where('tuanId',$id)
		           ->field('gu.tuanId,gu.tuanPrice,g.shopId,s.userId,g.goodsName,g.goodsSn,gu.tuanStatus,g.goodsId')->find();
		if((int)$rs['tuanId']==0)return WSTReturn("无效的商品");
		...
		try{
			...
			if($res!==false){
			    $this->modifyTuanPrice($rs['goodsId'], $rs['tuanPrice']);
				//发送一条商家信息
    ...
    protected function modifyTuanPrice($goodsId, $tuanPrice)
    {
        DB::name('goods_specs')->where('goodsId',$goodsId)->where('tuanPrice',0)->update(['tuanPrice'=>$tuanPrice]);
    }
