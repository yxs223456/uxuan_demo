...
class Ucenter extends Base{
	...
	public function afterEditPass($params){
		...
		$user = Db::name('users')->where(["userId"=>$userId])->field(["userId","loginName"])->find();

		//同步登录到UC
		$ucenter = new \addons\ucenter\api\UcenterApi();
  		$uid = $ucenter-> ucedit($user['loginName'],input("post.oldPass"),input("post.newPass"),'',0);
  		$rs = WSTReturn($uid);
	...
	public function afterEditEmail($params){
		...
		if($userEmail!=""){
			...
			//同步登录到UC
			$ucenter = new \addons\ucenter\api\UcenterApi();
			$uid = $ucenter-> ucedit($user['loginName'],$loginPwd,'',$user['userEmail'],0);
			$rs = WSTReturn($uid);
			if(is_string($uid))exit(json_encode($rs));
		}
	...
	public function afterEditUser($params){
		...
		//同步登录到UC
		$ucenter = new \addons\ucenter\api\UcenterApi();
		$uid = $ucenter-> ucedit($user['loginName'],'',$loginPwd,$user['userEmail'],1);
		$rs = WSTReturn($uid);
		if(is_string($uid))exit(json_encode($rs));
