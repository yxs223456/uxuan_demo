...
class File extends SplFileObject
{
    ...
    public function check($rule = [])
    {
        ...
        if (isset($rule['checkImg']) && $rule['checkImg'] == true && !$this->checkImg()) {
            return false;
        }
    ...
    public function checkImg()
    {
        $extension = strtolower(pathinfo($this->getInfo('name'), PATHINFO_EXTENSION));
        /* 对图像文件进行严格检测 */
        if (in_array($extension, ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf', 'mp4']) && !in_array($this->getImageType($this->filename), [1, 2, 3, 4, 6, 13])) {
