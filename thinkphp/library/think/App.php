...
class App implements \ArrayAccess
{
    ...
    public function run()
    {
        ...
        try {
            ...
            // 记录路由和请求信息
            if ($this->debug) {
                $this->log('[ ROUTE ] ' . json_encode($this->request->routeInfo(), JSON_UNESCAPED_UNICODE));
                $this->log('[ HEADER ] ' . json_encode($this->request->header(), JSON_UNESCAPED_UNICODE));
                $this->log('[ PARAM ] ' . json_encode($this->request->param(), JSON_UNESCAPED_UNICODE));
            }
            ...
            // 执行调度
            $data = $dispatch->run();
        } catch (\wstmart\common\exception\AppException $e) {
            $selfExceptionHandle = true;
            $data = [
                'status' => $e->getCode(),
                'msg' => $e->getMessage()
            ];
            ...
            if (!empty($_SERVER['HTTP_HOST']) && stripos($_SERVER['HTTP_HOST'], 'api.mryx.365uxuan.com') !== false) {
                $env = 'production';
            } elseif (!empty($_SERVER['SERVER_NAME']) && stripos($_SERVER['SERVER_NAME'], 'api.mryx.365uxuan.com') !== false) {
                $env = 'production';
            } else {
                $env = 'dev';
            }
        ...
        $this->middlewareDispatcher->add(function (Request $request, $next) use ($data, $selfExceptionHandle) {
            // 输出数据到客户端
            if ($data instanceof Response) {
                ...
                $type = $selfExceptionHandle ? $this->config('app.default_app_type') : $type;
