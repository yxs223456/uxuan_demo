...
class Users extends Base{
    ...
    {
        $user = $this->where(['inviteCode'=>$inviteCode])->find();
        if (empty($user)) {
            throw AE::factory(AE::USER_NOT_EXISTS);
        }
        return $user;
    ...
    public function getUser(array $map)
    {
     ...
     *判断用户个人信息是否完整
     */
    public function checkInformationComplete($userId)
    {
        $user = $this->getUserById($userId);
        if ($user->userSex == 0 || $user->nickname == '' || $user->userPhoto == '' || $user->brithday == null ||
            $user->brithday == '0000-00-00') {
            return false;
        }
        return true;
    }

    public function checkPhoneIsBind($phone)
    {
        try {
    ...
    {
        $user = $this->where(['userPhone'=>$phone])->find();
        return $user;
    }

    public function getUserByUnionid($unionid)
    {
        $user = $this->where(['wxUnionId'=>$unionid])->find();
        return $user;
    }

    public function createUserOnlyPhone($phone)
    {
        $userInfo = [
            'userPhone' => $phone,
            'inviteCode' => $this->createNewInviteCode(),
            'lastIP' => getClientIp(),
        ...
        return $this->insert($userInfo, false, true);
    }

    public function createUserOnlyPhoneAndWechat($phone, $wechatData)
    {
        $userInfo = [
            'userPhone' => $phone,
            'inviteCode' => $this->createNewInviteCode(),
        ...
        $data = array_merge($userInfo, $wechatData);
     ...
     * 用户签到
     */
    public function todaySign($userId)
    {
        $month = date('Y-m');
        $day = date('d');
        $signRecord = DB::name('signs')
            ->where('userId', $userId)
            ->where('month', $month)
            ->find();
        if (empty($signRecord)) {
            $signInfo = [
                'userId' => $userId,
                'month' => $month,
                'record' => json_encode([$day])
            ];
            DB::name('signs')->insert($signInfo);
        } else {
            $record = json_decode($signRecord['record'], true);
            $record[] = $day;
            $record = json_encode(array_unique($record));
            DB::name('signs')->where('id',$signRecord['id'])->data(['record'=>$record])->update();
        }
    }

    private function createNewInviteCode()
    {
        do {
            $inviteCode = $this->getInviteCodeNumber(6);
        } while($this->checkInviteCodeIsIn($inviteCode));
        return $inviteCode;
    }

    private function getInviteCodeNumber($length, $isNum = false)
    {
        $inviteCode = '';
        if ($isNum) {
            for ($i = 0; $i < $length; $i++) {
        ...
        $letters = '23456789ABCDEFGHJKLMNPQRSTUVWXY';
        $len = strlen($letters);
        for ($i = 0; $i < $length; $i++) {
            $inviteCode .= substr($letters, mt_rand(0, $len - 1), 1);
        }
        return $inviteCode;
    }

    private function checkInviteCodeIsIn($inviteCode)
    {
        if (empty($inviteCode)) {
            throw AE::factory(AE::COM_PARAMS_ERR);
        }
        $count = $this->where('inviteCode', $inviteCode)->count();
        return !!$count;
    }
    ...
    * 绑定邮箱
    ...
    public function editBindPhone($userId,$userPhone){
        $data = array();
        $data["userPhone"] = $userPhone;
        $rs = $this->update($data,['userId'=>$userId]);
        if(false !== $rs){
            ...
            throw AE::factory(AE::COM_MOBILE_BIND_FAIL);
        }
    }
    /**
     * 查询并加载用户资料
    ...
    {
        if (empty($token)) {
            throw AE::factory(AE::COM_TOKEN_ERROR);
        }
        $user = DB::name('app_session')->field('userId')
            ->where('tokenId', $token)
            ->find();
        if ($user) {
            return $user['userId'];
    ...
    public function getUserWechatInfo($tokenId)
    {
        $info = DB::name('app_session')->alias('as')
            ->join('users u', 'as.userId=u.userId', 'left')
            ->where('tokenId', $tokenId)
            ->where('userStatus', 1)
            ->field('u.userId, u.wxUnionId, u.wxOpenId, U.appOpenId')
            ->find();
        if (!$info) {
            throw AE::factory(AE::COM_USERS_ERROR);
        }
    ...
    public function getUserByToken($where)
    {
        $info = $this->where($where)->field('userId')->find();
        return $info;
    }
