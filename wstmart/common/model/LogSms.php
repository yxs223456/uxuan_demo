...
class LogSms extends Base{
    ...
    public function saveLog($phoneNumber, $smsFunc, $template, $params, $smsReturnCode, $ip, $userId=0, $smsSrc = 2) {
        $data['smsSrc'] = $smsSrc;
        $data['smsUserId'] = $userId;
        $data['smsPhoneNumber'] = $phoneNumber;
        $data['smsContent'] = $this->getSmsContent($template, $params);
        $data['smsReturnCode'] = $smsReturnCode;
        $data['smsIP'] = $ip;
        $data['smsFunc'] = $smsFunc;
        $data['createTime'] = date('Y-m-d H:i:s');
        $this->data($data)->save();
    ...
    public function getSmsContent($template, $params)
    {
