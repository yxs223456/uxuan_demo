...
class Systems extends Base{
	...
	public function getSysMessages(){
		...
		if(in_array('shoporder',$tasks)){
		    $data['shoporder']['239'] = Db::name('order_refunds')->where('shopId',$shopId)->where('refundStatus','in',[0,4])->count();
		    $data['shoporder']['24'] = Db::name('orders')->where(['shopId'=>$shopId,'orderStatus'=>0,'dataFlag'=>1])->where('afterSaleStatus','in',[0,3,5])->count();
		    ...
		    //在线支付的退款单
		    $data['shoporder']['45'] = Db::name('orders')->alias('o')->join('order_refunds orf','orf.orderId=o.orderId')->where(['o.shopId'=>$shopId,'refundStatus'=>0,'o.dataFlag'=>1])->count();
		    //获取库存预警数量
