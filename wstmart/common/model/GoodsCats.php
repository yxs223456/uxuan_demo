...
class GoodsCats extends Base{
	 ...
	 * 获取列表
	 */
	public function listQuery($parentId,$isFloor = -1,$isAudit=true){
	    $where['dataFlag'] = 1;
	    $where['isShow'] = 1;
	    $where['parentId'] = $parentId;
	    if ($isAudit) $where['isAudit'] = 0;
		$dbo = $this->where($where);
		if($isFloor!=-1)$dbo->where('isFloor',$isFloor);
		return $dbo->order('catSort asc')->select();
	}

	public function getAuditCats($isAudit, $field='catId')
    {
        $where['dataFlag'] = 1;
        $where['isShow'] = 1;
        //$where['isFloor'] = 1;
   ...
   /**
    ...
    public function getGoodsCatInfo($where, $field='catId')
    {
        $goodsCatInfo = $this->where($where)->field($field)->select()->toArray();
        return $goodsCatInfo;
    }
}
