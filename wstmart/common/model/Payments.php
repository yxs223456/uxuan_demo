...
class Payments extends Base{
     ...
     * 添加支付信息
     */
    public function insertWechatOrder($data)
    {
        if (!is_array($data) && empty($data)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $wayId = DB::name('wx_pay_orders')->insertGetId($data);
        if (!$wayId) {
            throw AE::factory(AE::WECHAT_INSERT_PAY_ORDERS_FAIL);
        }
        return $wayId;
    }
     ...
     * 添加微信退款信息
     */
    public function insertWechatRefundData($data)
    {
        if (!is_array($data) && empty($data)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $wayId = DB::name('wx_refund_orders')->insertGetId($data);
        if (!$wayId) {
            throw AE::factory(AE::WECHAT_INSERT_REFUND_ORDERS_FAIL);
        }
        return $wayId;
    }
     ...
     * 查询支付信息
     */
    public function getWechatOrder($where)
    {
        if (empty($where)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $wayId = DB::name('wx_pay_orders')->where($where)->find();
        return $wayId;
    }
     ...
     * 更新支付信息
     */
    public function updateWechatOrder($where, $data)
    {
        if (empty($where) && !is_array($data) && empty($data)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $wayId = DB::name('wx_pay_orders')->where($where)->update($data);
        return $wayId;
    }

    public function getWxPayOrderByOutTradeNo($outTradeNo)
    {
        $payOrder = DB::name('wx_pay_orders')->where('out_trade_no', $outTradeNo)->find();
        if (empty($payOrder)) {
     ...
     * 关联微信退款查询
     */
    public function getWxRefund($where)
    {
        if (empty($where)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $isPintuan = DB::name('wx_refund_orders')->where($where)->find();
        return $isPintuan;
    }
     ...
     * 更新退款信息
     */
    public function updateWechatRefundOrder($where, $data)
    {
        if (empty($where) && !is_array($data) && empty($data)) {
            throw AE::factory(AE::COM_PARAMS_EMPTY);
        }
        $wayId = DB::name('wx_refund_orders')->where($where)->update($data);
        return $wayId;
    }
}
