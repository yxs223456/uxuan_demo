...
use wstmart\common\service\Weixin;
...
class Payments extends Base{
    protected $beforeActionList = [
        'checkAuth'=> ['except'=>'delwechatpaycallbackurl,delwechatrefundcallbackurl']
    ];
    // 前置方法执行列表
    protected $openAction = [
        'checkAuth',
        'wechatpay',
        'delwechatpaycallbackurl',
        'wechatrefund',
    ];
    ...
    public function weChatPay()
    {
        ...
        $tradeStatus = getInput('tradeStatus', 1);
        if (empty($orderNo)) {
            throw AE::factory(AE::COM_PARAMS_ERR);
        }
        $weChat = new Weixin();
        $weChatPay = $weChat->weiXinUnifiedOrder($orderNo, $type, $tradeStatus);
     ...
     * 微信支付回调地址
     */
    public function delWeChatPayCallBackUrl()
    {
        //获取通知的数据
        $xml = file_get_contents("php://input");
        $weChat = new Weixin();
        $weChatPay = $weChat->delWeixinNotify($xml);
        return $weChatPay;
    }
