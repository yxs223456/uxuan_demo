...
class Goods extends CGoods{
	...
	public function add(){
        ...
        if (count($goodsCats) == 3) {
            $data['goodsSn'] = 'F'.$goodsCats[0].'S'.$goodsCats[1].'T'.$goodsCats[2].$data['goodsSn'];
            $data['productNo'] = 'F'.$goodsCats[0].'S'.$goodsCats[1].'T'.$goodsCats[2].$data['productNo'];
        } else {
            $data['goodsSn'] = 'F'.$goodsCats[0].'S'.$goodsCats[1].$data['goodsSn'];
            $data['productNo'] = 'F'.$goodsCats[0].'S'.$goodsCats[1].$data['productNo'];
        }
        if (!empty($data['goodTags'])) {
            $goodsLabel = explode(',', $data['goodTags']);
        }
		unset($data['goodTags']);
		Db::startTrans();
        try{
			...
			if(false !== $result){
				...
				if (!empty($goodsLabel)) {
                    $this->editGoodsLabel($goodsLabel, $goodsId);
                }
				//商品图片
    	        ...
    	        if($data['goodsType']==0 && $specsIds!=''){
		    		...
		    		$pinTuan = (new P())->getPintuanByGoodsId($goodsId);
		    		if (empty($pinTuan)) {
		    		    $tuanPrice = 0;
                    } else {
		    		    $tuanPrice = $pinTuan['tuanPrice'];
                    }
		    		foreach ($specsIds as $v){
		    			$vs = explode('-',$v);
		    			$goodsSpecIds = [];
                        ...
                        $gspec['tuanPrice'] = (float)Input('tuanPrice_'.$v)?(float)Input('tuanPrice_'.$v):$tuanPrice;
	...
	public function editGoodsLabel($goodsLabel, $goodsId)
    {
        $goodsLabelData = [];
        foreach ($goodsLabel as $label_v) {
            $goodsLabelData[] = ['goodsId'=>$goodsId, 'tagId'=>$label_v, 'createTime'=>date('Y-m-d H:i:s')];
        }
        Db::name('goods_labels')->insertAll($goodsLabelData);
    }

    public function deleteGoodsLabel($goodsId)
    {
        Db::name('goods_labels')->where('goodsId', $goodsId)->delete();
    }
    	        ...
    	        if($data['goodsType'] ==0 && $specsIds!=''){
		    		...
		    		foreach ($specsIds as $v){
			    		...
			    		if($defaultSpec==$v){
		    				...
		    				$defaultPrice = $gspec['specPrice'];
		    				if ($gspec['tuanPrice']) {
//                                $this->setTuanPrice($goodsId, $gspec['tuanPrice']);
                            }
			    		}else{
			    			$gspec['isDefault'] = 0;
			    		}
	...
	private function setTuanPrice($goodsId, $tuanPrice)
    {
        DB::name('pintuans')->where('goodsId',$goodsId)->data(['tuanPrice'=>$tuanPrice])->update();
    }
	...
	public function getById($goodsId){
		...
		if(!empty($rs)){
			...
			$rs['spec0'] = $spec0;
			$rs['spec1'] = $spec1;
			$rs['goodTags'] = Db::name('goods_labels')->where('goodsId', $goodsId)->column('tagId');
			//获取销售规格
			$rs['saleSpec'] = Db::name('goods_specs')->where('goodsId',$goodsId)->field('id,isDefault,productNo,specIds,marketPrice,specPrice,tuanPrice,specStock,warnStock,saleNum')->select();
			//获取属性值
