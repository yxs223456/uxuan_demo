...
function save(){
	...
	$('#editform').isValid(function(v){
		if(v){
            ...
            $("input[name='goodTags']:checked").each(function(){
                goodTags.push($(this).val());
            });
			var specmap = [];
...
function fillSepcSale(){
	...
	for(var i=0;i<OBJ.saleSpec.length;i++){
		...
		$('#tuanPrice_'+tmpids).val(OBJ.saleSpec[i].tuanPrice);
...
function addSpecSaleCol(){
	...
	for(var i=0;i<specsRows.length;i++){
		...
		html.push('  <td><input type="text" class="spec-sale-goodsNo j-ipt" id="productNo_'+id+'" value="'+specProductNo+'" onblur="checkProductNo(this)" ></td>',
	              ...
	              '  <td><input type="text" class="spec-sale-ipt j-ipt" id="tuanPrice_'+id+'" onblur="WST.limitDecimal(this,2);javascript:WST.limitDecimal(this,2)" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)"></td>',
		...
		if(json.status==1 && json.data){
			...
			if(json.data.spec0 || json.data.spec1){
				...
				html.push('<div id="specSaleHead" class="spec-head">销售规格</div>',
				          ...
				          '     <th><font color="red">*</font>拼团价<br/><input type="text" class="spec-sale-ipt" onblur="WST.limitDecimal(this,2);batchChange(this.value,\'tuanPrice\')" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)"></th>',
