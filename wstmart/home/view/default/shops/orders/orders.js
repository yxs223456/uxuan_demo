...
function refundByPage(p){
    ...
    params.key = $.trim($('#key').val());
    params.page = p;
    $.post(WST.U('home/orders/refundByPage'),params,function(data,textStatus){
        $('#loading').hide();
        var json = WST.toJson(data);
        $('.j-order-row').remove();
        if(json.status==1){
...
function refundGoods(id) {
    var ll = WST.load({msg:'正在加载信息，请稍候...'});
    $.post(WST.U('home/orders/toShopRefundGoods'),{id:id},function(data){
        layer.close(ll);
        var w = WST.open({
            type: 1,
            title:"退货退款操作",
            ...
            yes: function(index, layero){
                ...
                ll = WST.load({msg:'数据处理中，请稍候...'});
                $.post(WST.U('home/orderrefunds/shoprefundgoods'),params,function(data){
                    layer.close(ll);
                    var json = WST.toJson(data);
                    if(json.status==1){
                        WST.msg(json.msg, {icon: 1});
