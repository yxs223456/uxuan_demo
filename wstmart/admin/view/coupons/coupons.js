...
$(function () {
    ...
    var cols = [
        ...
        {title:'领取数量', name:'receiveNum', width:10, renderer:function (val,item,rowIndex) {
            return "<span><p class='wst-nowrap'>"+item['receiveNum']+"</p> </span>"
        }},
        {title:'发放对象', name:'grantObjects', width:10, renderer:function (val,item,rowIndex) {
            return "<span><p class='wst-nowrap'>"+item['grantObjects']+"</p> </span>"
        }},
        {title:'操作', name:'op' ,width:50, align:'center', renderer: function(val,item,rowIndex){
            var h = "";
            if(WST.GRANT.XSLB_01)h += "<a class='btn btn-blue' href='javascript:toEdit(" + item['couponId'] + ")' ><i class='fa fa-pencil'></i>修改</a> ";
            if(WST.GRANT.XSLB_01)h += "<a class='btn btn-red' href='javascript:toDel(" + item['couponId'] + ")'><i class='fa fa-trash-o'></i>删除</a> ";
            return h;
        }}
    ];
    mmg = $('.mmg').mmGrid({height: h-85,indexCol: true, cols: cols,method:'POST',
        url: WST.U('admin/coupons/pageQuery'), fullWidthRows: true, autoLoad: true,
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
...
function toEdit(id){
    location.href = WST.U('admin/coupons/toEdit','couponId='+id);
}
...
function toDel(id) {
    var box = WST.confirm({
        content: "您确定要删除该红包吗?", yes: function () {
            ...
            $.post(WST.U('admin/coupons/del'), {couponId: id}, function (data, textStatus) {
                layer.close(loading);
                var json = WST.toAdminJson(data);
                if (json.status == '1') {
                    WST.msg("操作成功", {icon: 1});
                    layer.close(box);
                    location.href=WST.U('Admin/coupons/index');
                } else {
...
function editInit(){
    ...
    $('#couponForm').validator({
        ...
        valid: function(form){
            ...
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('admin/coupons/'+((params.couponId==0)?"add":"edit")),params,function(data,textStatus){
                layer.close(loading);
                var json = WST.toAdminJson(data);
                if(json.status=='1'){
                    WST.msg("操作成功",{icon:1});
