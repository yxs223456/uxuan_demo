...
function initSaleGrid(){
    ...
    var cols = [
        	...
        	{title:'商品ID', name:'goodsId' ,width:10,sortable:true},
        	...
        	{title:'权重', name:'weight' ,width:10,renderer: function(val,item,rowIndex){
                return "<input class='form-control goodsWeight' style='width:50px' idValue='"+item['goodsId']+"' value="+item['weight']+">";
            }},
			{title:'屏蔽审核', name:'isAudit', width: 10,renderer: function(val,item,rowIndex){
				return '<input type="checkbox" '+((item['isAudit']==1)?"checked":"")+' id="isAudit" name="isAudit" value="1" class="ipt" lay-skin="switch" lay-filter="isAudit" data="'+item['goodsId']+'" lay-text="显示|隐藏">'
			}},
    ...
    $(document).on('change', '.goodsWeight', function () {
    	$this = $(this);
        ...
        $.ajax({
			async: false,
			type: 'get',
			url: '/admin/goods/modifyWeight',
			data: {'goodsId': $id,'weight': $weight},
    ...
    mmg.on('loadSuccess',function(data){
        layui.form.render();
        layui.form.on('switch(isAudit)', function(data){
...
function toggleIsShow( goodsId, isAudit){
    $.post(WST.U('admin/goods/setToggle'), {'goodsId':goodsId, 'isAudit':isAudit}, function(data, textStatus){
