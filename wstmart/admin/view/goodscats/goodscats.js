...
function initGrid(){	
	grid = $('#maingrid').WSTGridTree({
        ...
        rownumbers:true,
        columns: [
	        { display: '分类名称', width: 200,name: 'catName', id:'catId', align: 'left',isSort: false,render: function (item)
                {
            ...
            { display: '推荐楼层', width: 50, name: 'isFloor',isSort: false,
            ...
            { display: '是否屏蔽', width: 50, name: 'isAudit',isSort: false,
                render: function (item)
                {
                    return ((item["isAudit"]==1)?'是':'否');
                }
            },
...
function initUpload(){
	...
	WST.upload({
	    ...
	    accept: {extensions: 'gif,jpg,jpeg,png,mp4',mimeTypes: 'image/mp4,image/jpg,image/jpeg,image/png,image/gif'},
	    callback:function(f){
