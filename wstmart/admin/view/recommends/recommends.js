...
/**指定商品页**/
function loadGoodsActivity(suffix){
    var params = WST.getParams('.ipt'+suffix);
    params.key = params['key'+suffix];
    params.goodsCatId = WST.ITGetGoodsCatVal('pgoodsCats1'+suffix);
    if(params.goodsCatId==''){
...
function listQueryByGoodsActivity(suffix){
    suffix = (typeof(suffix)=='object')?'_2':suffix;
    $('#rlist'+suffix).empty();
    $('#ids'+suffix).val('');
    var params = {};
    params.dataType = $('#dataType'+suffix).val();
    params.type2 = $('#type2'+suffix).val();
    params.goodsCatId = WST.ITGetGoodsCatVal('pgoodsCats2'+suffix);
    var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(WST.U('admin/recommends/listQueryByGoodsActivity'),params,function(data,textStatus){
        layer.close(loading);
        var json = WST.toAdminJson(data);
        if(json.status=='1'){
            if(json.data && json.data.length){
                ...
                for(var i=0;i<json.length;i++){
                    ...
                    html.push('<div class="trow"><div class="tck"><input type="checkbox" name="rchk'+suffix+'" class="rchk'+suffix+'" value="'+data.dataId+'"></div>');
                    html.push('<div class="ttxt">【'+data.shopName+'】'+data.goodsName+'</div>');
                    html.push('<div class="top"><input type="text" class="s-sort s-ipt'+suffix+'" value="0" v="'+data.dataId+'"></div></div>');
                }
                $('#ids'+suffix).val(ids.join(','));
                $("#rlist"+suffix).html(html.join(''));
            }
...
function editGoodsActivity(suffix){
    var params = {},ids = [];
    $('input[name="rchk'+suffix+'"]').each(function(){
        ids.push($(this).val());
    });
    ...
    params.ids = ids.join(',');
    params.type2 = $('#type2'+suffix).val();
    params.title = $('#title'+suffix).val();
    params.bannerIsShow = $('#bannerIsShow'+suffix).val();
    params.banner = $('#banner'+suffix).val();
    params.link = $('#link'+suffix).val();

    var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(WST.U('admin/recommends/editGoodsActivity'),params,function(data,textStatus){
        layer.close(loading);
        var json = WST.toAdminJson(data);
        if(json.status=='1'){
            WST.msg("保存成功",{icon:1});
    ...
    WST.upload({
        ...
        callback:function(f){
            ...
            if(json.status==1){
                ...
                //保存上传的图片路径
                $('#banner_2').val(json.route+json.name);
