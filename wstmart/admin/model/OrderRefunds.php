...
use think\Db;
use Env;
use wstmart\common\service\Weixin as W;
/**
 * ============================================================================
 * WSTMart多用户商城
...
//		$where[] = ['orderStatus','in',[-1,-3]];
	...
	public function getInfoByRefund(){
		return $this->alias('orf')->join('__ORDERS__ o','orf.orderId=o.orderId')->where([['orf.id','=',(int)input('get.id')],['isRefund','=',0],['refundStatus','=',1]])
...
//	}
    public function orderRefund(){
        ...
        $order = model('orders')->get($refund->orderId);
        if($order->payType==1){
            $rs = $w->wechatRefundFlow($refund->orderId, 2);
        }else{
            return WSTReturn("目前只支持微信方式退款!");
        }
	...
	public function complateOrderRefund($obj){
        ...
        try{
			...
			$order = model('orders')->get($refund->orderId);
			if(!in_array($order->orderStatus,[-1,-3]) && ($order->isPay==1 || ($order->payType==0 && $order->useStarfish>0)))return WSTReturn("无效的退款订单!");
			//修改退款单信息
