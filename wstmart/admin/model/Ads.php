...
use think\Db;
use wstmart\admin\model\Goods;
use wstmart\admin\model\GoodsCats;
use wstmart\admin\model\GoodsTags;
/**
 * ============================================================================
 * WSTMart多用户商城
...
class ads extends Base{
	...
	public function pageQuery(){
		...
		return Db::name('ads')->alias('a')
					...
					->field('adId,adName,adPositionId,adURL,adStartDate,adEndDate,adPositionId,adFile,adClickNum,ap.positionName,a.adSort,a.isShow,a.moduleUrl')
	...
	public function add(){
		...
		if ($data['accessType']==1) {
		    if (in_array($data['accessModuleType'], [1,2,3])) {
                $check = $this->checkModuleId($data['accessModuleType'], $data['moduleId']);
                if (!$check) return WSTReturn('此类型id不存在',-1);
            }
		    $data['moduleUrl'] = 'key='.$data['accessModuleType'].'&value='.$data['moduleId'];
            unset($data['accessModuleType'],$data['moduleId']);
        } else {
            $data['moduleUrl'] = '';
		    unset($data['accessModuleType'],$data['moduleId']);
        }
	...
	public function checkModuleId($type,$id)
    {
        $check = true;
        if ($type==1) {
            $info = (new Goods())->getGoodsInfo($id);
            $check = !$info ? false : true;
        } elseif ($type==2) {
            $info = (new GoodsCats())->getGoodscats($id);
            $check = !$info ? false : true;
        } elseif ($type==3) {
            $info = (new GoodsTags())->getById($id);
            $check = !$info ? false : true;
        }
        return $check;
	...
	*/
    ...
    {
        $adId = input('post.adId/d');
        $isShow = input('post.isShow/d');
        $result = $this->where(['adId'=>$adId])->setField("isShow", $isShow);
        if(false !== $result){
