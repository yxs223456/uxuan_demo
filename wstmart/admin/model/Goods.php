...
class Goods extends Base{
	...
	public function saleByPage(){
		...
		$rs = $this->alias('g')->join('__SHOPS__ s','g.shopId=s.shopId','left')
		    ->where($where)
			->field('goodsId,goodsName,goodsSn,saleNum,shopPrice,g.shopId,goodsImg,s.shopName,goodsCatIdPath,weight,isAudit')
			->order($order)
    ...
    {
        $info = $this->where(['goodsId'=>$goodsId, 'dataFlag'=>1])->find();
        return $info;
    }

	public function modifyWeight()
    {
        $goodsId = input('goodsId');
        $weight = input('weight');
        if (!checkInt($weight, true, false)) {
            return WSTReturn('权重值只能为整数',-1);
        }
        if (!checkInt($goodsId, false)) {
            return WSTReturn('参数错误',-1);
        }
        $this->where('goodsId', $goodsId)->update(['weight'=>$weight]);
	...
	*/
    ...
    public function setToggle()
    {
        ...
        $isAudit = input('post.isAudit/d');
        $result = $this->where(['goodsId'=>$goodsId])->setField("isAudit", $isAudit);
        if(false !== $result){
