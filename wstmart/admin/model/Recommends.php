...
class Recommends extends Base{
	...
	public function listQueryByGoodsActivity(){
        $dataType = (int)input('post.dataType');
        $goodsCatId = (int)input('post.goodsCatId');
        $type2 = input('post.type2');
        $goodsIds = Db::name('goods_activity')->alias('ga')
            ->leftJoin('goods_activity_goods gag', 'ga.id=gag.activityId')
            ->where('ga.type2', $type2)->column('gag.goodsId');
        if (count($goodsIds)) {
            $rs = DB::name('goods')->alias('g')
                ->join('__SHOPS__ s','s.shopId=g.shopId','inner')
                ->whereIn('g.goodsId', $goodsIds)
                ->field('g.goodsId dataId,g.goodsName,s.shopName')->select();
            return $rs;
        } else {
            return [];
        }
     ...
     * 推荐商品
     */
    public function editGoodsActivity(){
        $ids = input('post.ids');
        $type2 = input('post.type2');
        $title = input('post.title');
        $banner = input('post.banner');
        $bannerIsShow = input('post.bannerIsShow');
        $link = input('post.link');
        $ids = explode(',',$ids);
        Db::startTrans();
        try{
            DB::name('goods_activity')->where('type2', $type2)->update([
                'title' => $title,
                'banner' => $banner,
                'bannerIsShow' => $bannerIsShow,
                'link' => $link
            ]);
            $activity = DB::name('goods_activity')->where('type2', $type2)->field('id')->find();
            DB::name('goods_activity_goods')->where('activityId', $activity['id'])->delete();
            if(!empty($ids)){
                $data = [];
                foreach ($ids as $id){
                ...
                DB::name('goods_activity_goods')->insertAll($data);
            }
            Db::commit();
            return WSTReturn("提交成功", 1);
        }catch(\Exception $e) {
