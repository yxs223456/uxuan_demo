...
class Reports extends Base{
    public function offlineStatByPage(){
        ...
        $usersStat = Db::name('users')
            ->where('pid', 'like', $channel . '\_%')
            ->group('pid')
            ->field('count(*) total,pid')
            ->whereTime('bindPidTime','between',[$start,$end])
            ->select();
        $usersStat = array_column($usersStat, 'total', 'pid');
        $usersStatTotal = array_sum($usersStat);

        $ordersSuccessStat = Db::name('orders')
            ->where('pid', 'like', $channel . '\_%')
            ->where('orderStatus', '>=', '0')
            ->group('pid')
            ->field('count(*) total,pid')
            ...
            ->select();
        $ordersSuccessStat = array_column($ordersSuccessStat, 'total', 'pid');
        $ordersSuccessStatTotal = array_sum($ordersSuccessStat);

        $ordersUnSuccessStat = Db::name('orders')
            ->where('pid', 'like', $channel . '\_%')
            ->where('orderStatus', '<', '0')
            ->group('pid')
            ->field('count(*) total,pid')
            ->whereTime('createTime','between',[$start,$end])
            ->select();
        $ordersUnSuccessStat = array_column($ordersUnSuccessStat, 'total', 'pid');
        $ordersUnSuccessStatTotal = array_sum($ordersUnSuccessStat);

        $rs['data'] = [
            [
                'pid'=>'总计',
                'bindUserNum'=>$usersStatTotal,
                'successOrderNum'=>$ordersSuccessStatTotal,
                'unSuccessOrderNum'=>$ordersUnSuccessStatTotal
            ]
        ];
        if ($channel == '666666') {
            for($i = 1; $i <= 27; $i++) {
                $pid = '666666_' . $i;
                $rs['data'][$i]['pid'] = $pid;
                $rs['data'][$i]['bindUserNum'] = $usersStat[$pid]??0;
                $rs['data'][$i]['successOrderNum'] = $ordersSuccessStat[$pid]??0;
                $rs['data'][$i]['unSuccessOrderNum'] = $ordersUnSuccessStat[$pid]??0;
            }
        } else {
            for($i = 0; $i <= 20; $i++) {
                $pid = '666667_' . $i;
